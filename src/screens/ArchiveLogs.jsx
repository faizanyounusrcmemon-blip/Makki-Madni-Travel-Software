...
export default function ArchiveLogs({ 
  ...
  const load = async () => {
    try {
      ...
      // Pointing directly to logs endpoint payload mapping
      const res = await axios.get(
        `/api/archive/logs/${archiveId}`
      );
  ...
  // ✅ FIX: Moved the null validation BEFORE rendering 'rows' array to avoid runtime crashes
  if (!log) {
    return (
      <div className="container-fluid p-4">
        <div className="alert alert-warning">
          No delete log found for this archive
        </div>
        <button
  ...
  // Rows configuration mapping safe values
  const rows = [
        ...
        <div className="card-body">
          ...
          <div className="alert alert-secondary border-0 mb-0">
            ...
            <span className="ms-2 fw-semibold">
              {new Date(
                log.deleted_at || log.archived_at
              ).toLocaleString()}
            </span>
          </div>
