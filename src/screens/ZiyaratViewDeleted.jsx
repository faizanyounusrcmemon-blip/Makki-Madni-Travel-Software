...
export default function ZiyaratViewDeleted({ id, onNavigate }) {
  const [data, setData] = useState(null);
  const ref = useRef(null);

const { exportPDF, printPDF } = usePdf(ref, {
  filePrefix: "ZiyaratDelete",
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/ziyarat/get-deleted/${id}`)
      .then((r) => r.json())
      .then((res) => {
...
>
  🖨️ Print
      ...
      >
        ...
        {/* ===== ZIYARAT ROWS ===== */}
        <h5 className="fw-bold text-primary mb-2">Ziyarat Entries</h5>
        {data.rows.length === 0 && <p className="text-muted">No ziyarat rows</p>}
        {data.rows.map((r, i) => (
          <div key={i} className="border rounded p-2 mb-2 shadow-sm d-flex justify-content-between">
            <div>{r.description || r.text || r.route}</div>
