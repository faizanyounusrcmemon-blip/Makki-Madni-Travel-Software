import React, { useEffect, useState } from "react";

// ================= DATE FORMAT HELPER =================
const formatDate = (d) => {
  const date = new Date(d);
  const options = { day: "2-digit", month: "short", year: "numeric" };
  return date.toLocaleDateString("en-US", options);
};

export default function SupplierMissing({ onNavigate }) {
  const [rows, setRows] = useState([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    loadMissing();
  }, []);

  const loadMissing = async () => {
    try {
      setLoading(true);

      // 1️⃣ Supplier Missing API
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/purchase/missing-supplier`
      );
      const data = await res.json();
      if (!data.success) return;

      const missingRows = data.rows;

      // 2️⃣ Sale amounts from /reports/all
      const reportsRes = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/reports/all`
      );
      const reportsData = await reportsRes.json();

      const saleMap = {};
      reportsData.forEach((r) => {
        saleMap[r.ref_no] = r.total_pkr || 0;
      });

      // 3️⃣ Merge sale amount
      const finalRows = missingRows.map((r) => ({
        ...r,
        sale_pkr: saleMap[r.ref_no] || 0,
      }));

      setRows(finalRows);
      setLoading(false);
    } catch (err) {
      console.error("Error loading supplier missing:", err);
      setLoading(false);
    }
  };

  // ================= FILTER =================
  const filteredRows = rows.filter((r) => {
    if (!search) return true;
    const q = search.toLowerCase();
    const ref = String(r.ref_no || "").toLowerCase();
    const name = String(r.customer_name || "").toLowerCase();
    return ref.includes(q) || name.includes(q);
  });

  return (
    <div className="container p-3">
      <button
        className="btn btn-secondary btn-sm mb-3"
        onClick={() => onNavigate("dashboard")}
      >
        ⬅ Back
      </button>

      <h4 className="fw-bold text-info mb-3">
        🧾 Completed Purchases (Supplier Missing)
      </h4>

      <input
        type="text"
        placeholder="Search Ref No or Customer..."
        className="form-control form-control-sm mb-3"
        value={search}
        onChange={(e) => setSearch(e.target.value)}
      />

      {loading ? (
        <div className="text-center text-muted">Loading...</div>
      ) : (
        <div className="table-responsive shadow-sm rounded">
          <table className="table table-bordered table-hover table-sm align-middle mb-0">
            <thead className="table-dark">
              <tr>
                <th>Ref No</th>
                <th>Customer</th>
                <th>Supplier</th>
                <th className="text-end">Sale Amount (PKR)</th>
                <th className="text-end">Purchase Amount (PKR)</th>
                <th>Status</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              {filteredRows.length === 0 && (
                <tr>
                  <td colSpan="7" className="text-center text-success">
                    🎉 No supplier missing
                  </td>
                </tr>
              )}

              {filteredRows.map((r, i) => (
                <tr key={i} className="align-middle">
                  <td className="fw-bold text-primary">{r.ref_no}</td>

                  <td className="fw-semibold">
                    {r.customer_name || "-"}
                  </td>

                  <td className="text-danger fw-bold">
                    Not Added
                  </td>

                  <td className="text-end fw-bold text-success">
                    {r.sale_pkr
                      ? Number(r.sale_pkr).toLocaleString("en-US")
                      : "0"}
                  </td>

                  <td className="text-end fw-bold text-primary">
                    {r.total_amount
                      ? Number(r.total_amount).toLocaleString("en-US")
                      : "0"}
                  </td>

                  <td>
                    <span className="badge bg-info text-dark">
                      Supplier Missing
                    </span>
                  </td>

                  <td>
                    <button
                      className="btn btn-sm btn-primary"
                      onClick={() => onNavigate("purchase", r.ref_no)}
                    >
                      ➕ Add Supplier
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
